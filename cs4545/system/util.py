...
@click.argument('k_connected', type=int)
...
def compose(num_nodes, k_connected, topology_file, algorithm, template_file):
    prepare_compose_file(num_nodes, k_connected, topology_file, algorithm, template_file)


def prepare_compose_file(num_nodes, k_connected, topology_file, algorithm, template_file, location='cs4545'):
    with open(template_file, 'r') as f:
        ...
        for i in range(num_nodes):
            ...
            n['environment']['STARTER'] = 'true' if i == 0 else 'false'
            nodes[f'node{i}'] = n


            connections[i] = topology[i]
